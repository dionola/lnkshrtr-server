...
export const createLinkSchema = z
  ...
  .refine(
    (data) => !data.isPasswordProtected || (!!data.password && data.password.length >= 6),
    { message: 'Password must be at least 6 characters when protection is enabled', path: ['password'] },
  );
...
export const updateLinkSchema = z
  ...
  .refine(
    (data) => !(data.isPasswordProtected === true && data.password != null) || data.password.length >= 6,
