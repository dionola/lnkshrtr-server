...
describe("GET /api/links", () => {
  ...
  it("returns links ordered newest first", async () => {
    ...
    // Slight delay to get different createdAt
  ...
  it("returns 401 without Authorization header", async () => {
    const res = await request(getHttpServer()).get("/api/links");
...
describe("PUT /api/links/:id", () => {
  ...
  it("partial update — untouched fields remain unchanged", async () => {
    ...
    expect(res.body.isPublic).toBe(false); // unchanged
  });
...
describe("DELETE /api/links/:id", () => {
  ...
  it("returns 401 without auth", async () => {
    ...
    const res = await request(getHttpServer()).delete(`/api/links/${link.id}`);
...
describe("GET /api/links/code/:shortCode", () => {
  it("returns link by short code without auth", async () => {
    ...
    const res = await request(getHttpServer()).get("/api/links/code/pub001");
  ...
  it("never returns the password field", async () => {
    ...
    const res = await request(getHttpServer()).get("/api/links/code/pub002");
  ...
  it("returns the isPasswordProtected flag", async () => {
    ...
    const res = await request(getHttpServer()).get("/api/links/code/pub005");
  ...
  it("returns private links (frontend handles gating)", async () => {
    ...
    const res = await request(getHttpServer()).get("/api/links/code/pub003");
  ...
  it("returns inactive links (frontend handles gating)", async () => {
    ...
    const res = await request(getHttpServer()).get("/api/links/code/pub004");
  ...
  it("returns 404 for an unknown short code", async () => {
    const res = await request(getHttpServer()).get("/api/links/code/unknown");
...
describe("POST /api/links/code/:shortCode/click", () => {
  it("returns 200 and increments visit count", async () => {
    ...
    const res = await request(getHttpServer()).post("/api/links/code/clk001/click");

    expect(res.status).toBe(200);

    // Wait for the async increment to complete
  ...
  it("returns 200 even for a non-existent short code (silent fail)", async () => {
    const res = await request(getHttpServer()).post("/api/links/code/nonexistent/click");
  ...
  it("does not require authentication", async () => {
    ...
    const res = await request(getHttpServer()).post("/api/links/code/clk002/click");
