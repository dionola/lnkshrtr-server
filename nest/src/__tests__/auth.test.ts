...
describe("POST /api/auth/signup", () => {
  it("registers a new user and returns user + accessToken", async () => {
    const res = await request(getHttpServer()).post("/api/auth/signup").send({
      username: "alice",
  ...
  it("password is hashed in the database", async () => {
    await request(getHttpServer()).post("/api/auth/signup").send({
      username: "hashcheck",
...
describe("POST /api/auth/login", () => {
  it("returns user and accessToken with valid credentials", async () => {
    ...
    const res = await request(getHttpServer()).post("/api/auth/login").send({
  ...
  it("does not distinguish between wrong email vs wrong password (timing-safe message)", async () => {
    ...
    const badEmailRes = await request(getHttpServer()).post("/api/auth/login").send({
      email: "fake@example.com",
      password: "realpass",
    });
    const badPassRes = await request(getHttpServer()).post("/api/auth/login").send({
...
describe("GET /api/auth/me", () => {
  ...
  it("returns 401 when no Authorization header", async () => {
    const res = await request(getHttpServer()).get("/api/auth/me");
  ...
  it("returns 401 when token references a deleted user", async () => {
    // Sign a token with a random UUID that doesn't exist in DB
...
describe("GET /api/auth/google", () => {
  it("returns 501 when Google OAuth is not configured", async () => {
    // Our test env has no GOOGLE_CLIENT_ID/SECRET set
    const res = await request(getHttpServer()).get("/api/auth/google");
